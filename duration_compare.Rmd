...
data = read_csv("C:/Users/Xiaoyang Li/Desktop/data/App/appUS1.csv") %>% 
  dplyr::select(TimeToDoUS, TimeToDoUS2,Whichdepartmentperformedtheultrasoundforthisform, Whichdepartmentperformedtheultrasoundforthisform_A) %>% 
...
data1 = data %>% dplyr::select(time1, depar1) %>% rename(time = time1, depar = depar1)
data2 = data %>% dplyr::select(time2, depar2) %>% rename(time = time2, depar = depar2)

data = rbind(data1, data2)
data = drop_na(data) %>% filter(time >0)
...
data = drop_na(data)%>% filter(time1 >0 &time2 > 0)
